...
class Network:
    ...
    def changeWeights(self):    # correct the weights
        ...
        hiddenLayerWeightChange = np.matmul( hiddenLayerError.reshape((self.numHidden,1)),
                                             self.inputs.reshape((self.numInputs,1)).T   ).T  * self.learningRate

        self.hiddenLayerWeights = np.add(self.hiddenLayerWeights, hiddenLayerWeightChange)
        self.outputLayerWeights = np.add(self.outputLayerWeights, outputLayerWeightChange)  # changing weights
    ...
    def checkAccuracy(self):
        ...
        print(f"Accurate diagnoses = {100*acc_diagnoses} %")
        print(f"Sensitivity = {sensitivity}")
        print(f"Specificity = {specificity}")
        print(f"MSE-error = {meanSquared_error}")
        print("----")
